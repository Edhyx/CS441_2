...
public class ControllerAlbumManagement extends Controller {
	...
	public void notifyChangement(String changement) {
		if (changement.charAt(0) == '9') { // quitter l'application
			...
			// On test tout les codes possibles envoy�s par view
			if (changement.substring(0, 3).equals("NEW")) { // creation nouvel album
				...
				ArrayList<String> returns = model.getAllFilesWithNoFolder(
						changement.substring(changement.indexOf(" ") + 1, changement.length()));
				if (returns == null) {
					view.displayAlbumManagement(model.getFolderList());
			...
			} else if (changement.substring(0, 3).equals("ADF")) { // ajout d'une image a un album
	...
	private boolean newFolder(String data) { 		
		// methode utilisee pour la creation d'un album
		try {
	...
	private boolean addFileToFolder(String str) { 		
		...
		try {
			...
			if (i == 4) {
				return model.addFileToFolder(element[0], Integer.parseInt(element[1]), element[2], element[3]);
			} else {
				return false;
			}
		} catch (StringIndexOutOfBoundsException e) {
